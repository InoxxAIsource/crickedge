...
OUTPUT_PATH = os.path.join(PROCESSED_DIR, "realistic_edge_simulation_results.csv")
...
def run_simulation(rows, threshold, scenario="gross"):
    seen_matches = set()
    trades = []
...
def main():
    ...
    print(f"\n=== build_realistic_simulation.py ===")
    print(f"  Input rows: {len(all_rows)}")
    print(f"  Unique matches: {len(set(r['match_id'] for r in all_rows))}")
    print(f"\n  Friction parameters:")
    print(f"    Commission rate     : {COMMISSION_RATE:.0%}")
    print(f"    Slippage            : {SLIPPAGE_TICKS} tick(s) × {TICK_SIZE} = {SLIPPAGE_TICKS * TICK_SIZE:.2f}")
    print(f"    Execution delay     : {EXECUTION_DELAY_PENALTY:.0%} edge penalty")
    print(f"    Worst-case commission: {COMMISSION_RATE * 1.5:.1%}")
    print(f"    Worst-case slippage : {SLIPPAGE_TICKS * 2} ticks")
    ...
    for scenario in scenarios:
        for threshold in THRESHOLDS:
            result = run_simulation(all_rows, threshold, scenario)
    ...
    for scenario in scenarios:
        ...
        print(f"  {'Threshold':>10s} {'Trades':>7s} {'Wins':>6s} {'Win%':>7s} "
              f"{'Profit':>8s} {'ROI%':>7s} {'MaxDD':>7s} {'Comm':>6s}")
        print(f"  {'-'*10} {'-'*7} {'-'*6} {'-'*7} {'-'*8} {'-'*7} {'-'*7} {'-'*6}")
        for r in results:
            if r["scenario"] != scenario:
                continue
            print(f"  {r['threshold']:>10.0%} {r['total_trades']:>7d} {r['wins']:>6d} "
                  f"{r['win_rate']:>7.1%} {r['profit']:>8.2f} {r['roi_pct']:>6.1f}% "
                  f"{r['max_drawdown']:>7.2f} {r['total_commission']:>6.2f}")
